...
import {
  ...
  getQuoteComparisonDetailedReport,
  type QuoteComparisonDetailedReport,
...
export default function QuoteComparisonReportPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getQuoteComparisonDetailedReport(quoteId);
      setReport(data);
    } catch (err) {
  ...
  const onDownloadExcel = async () => {
    try {
      ...
      a.href = url;
      a.download = `quote_${quoteId}_karsilastirma_raporu.xlsx`;
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "10px" }}>
          <div>
            ...
            <div style={{ color: "#64748b", marginTop: "4px" }}>
              {report.quote.title} • Teklif #{report.quote.id}
            </div>
          </div>
